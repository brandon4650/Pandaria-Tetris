...
if not C_Timer then
    ...
    -- Create a frame for handling timers
    ...
    function C_Timer.After(delay, callback)
        ...
        table.insert(timerQueue, timer)
        
        if #timerQueue == 1 then
            timerFrame:SetScript("OnUpdate", function(self, elapsed)
                local currentTime = GetTime()
                local i = 1
                while i <= #timerQueue do
                    local timer = timerQueue[i]
                    if currentTime >= timer.expires then
                ...
                -- Stop the OnUpdate if no more timers
